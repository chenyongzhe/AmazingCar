...
amazing_car::my_location_msg decode(string location_str);
...
void calibration_anchor(){
	int choose = 1;
	while(choose){
		ROS_INFO("Please input 4 anchors' location");
		for(int i = 0;i < 4;i++){
			float x,y,z;
			scanf("%f%f%f",&x,&y,&z);
			anchorArray[i] = { x, y, z };
		}
		printf("first anchor:  x: %f, y: %f, z: %f\n", anchorArray[0].x, anchorArray[0].y, anchorArray[0].z);
		printf("second anchor: x: %f, y: %f, z: %f\n", anchorArray[1].x, anchorArray[1].y, anchorArray[1].z);
		printf("third anchor:  x: %f, y: %f, z: %f\n", anchorArray[2].x, anchorArray[2].y, anchorArray[2].z);
		printf("forth anchor:  x: %f, y: %f, z: %f\n", anchorArray[3].x, anchorArray[3].y, anchorArray[3].z);
		printf("sure? (Y/y/other_char)");
		char choose_c;
		scanf("%c", &choose_c);
...
int main(int argc, char ** argv){
	ros::init(argc, argv, "my_car_front_location_publisher");
	ros::NodeHandle n;
	string location_str;
	ros::Publisher location_pub = n.advertise<amazing_car::my_location_msg>("my_car_front_location", 1000);

	anchorArray[0].x = 0;anchorArray[0].y = 0;anchorArray[0].z = 2.11;
	anchorArray[1].x = 2.4;anchorArray[1].y = 0;anchorArray[1].z = 2.11;
	anchorArray[2].x = 2.4;anchorArray[2].y = 4.8;anchorArray[2].z = 2.11;
	anchorArray[3].x = 0;anchorArray[3].y = 4.8;anchorArray[3].z = 2.11;
	...
	while(ros::ok()){
		location_str.clear();
		my_serial.readline(location_str);
		//ROS_INFO("%s", location_str.c_str());
		//decode
		amazing_car::my_location_msg msg = decode(location_str);
...
amazing_car::my_location_msg decode(string location_str){
	...
	vec.clear();
	std::string delim = " ";
	split(location_str, delim, &vec);
	//ROS_INFO("%s", location_str.c_str());
	//ROS_INFO("%s", vec[0].c_str());
	int distanceTagToAnchor[4] = { 0 };
	int res = strcmp(vec[0].c_str(),"mc");
	//ROS_INFO("%s  %d", vec[0].c_str(), res);
	if (res == 0){
		for (int i = 0; i < 4; i++){
			sscanf(vec[i + 2].c_str(), "%x", &distanceTagToAnchor[i]);
			//printf("%d ", distanceTagToAnchor[i]);
		}
		//printf("\n");
	} else {
		msg.x = -10000;
	...
	GetLocation(result, 1, anchorArray, distanceTagToAnchor);
	msg.x = result->x * 1000;
	msg.y = result->y * 1000;
